# state and action are normalized
...
def diff_lim(state, action, t):
    ...
    height = h1+h2
    ang_lead = s2-c2
    speed = tf.sqrt(pow(state[4], 2) + pow(state[5]/2, 2)) 
...
def loc_torque(state, action, t):
    ...
    speed = pow(o1, 2) + pow(o2, 2)/2
    torque1 = -action[0] * o1 * 50
    torque2 = action[0] * o2 * 20 - pow(action[0], 2) * 5
